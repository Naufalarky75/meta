...
def logo():
 ...
 ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║META FACEBOOK METHOD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : Naufal Arky                   {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : 082360108860           {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/Naufalarky75 {N}  ║")
...
def yayanxd():
    ...
    print(' [%s1%s] Login with EAAB Token (%sOFF%s)'%(H,N,H,N))
    print(' [%s2%s] Login with Cookies (%sON%s)'%(H,N,H,N))
    print(' [%s3%s] Login with ID & Password (%sOFF%s)'%(H,N,H,N))
        ...
        if ___yayanganteng___ in ('raraY', 'raray'):
            ...
            while True:
                ...
                if pwek == '':
                    ...
                    __yan__(pwek.split(','))
                    break
        elif ___yayanganteng___ in ('METAROYID', 'Metaroyid', 'metaroyid'):
            self.tampilkan_apk()
            mentod()
            self.__pler__()
        else:
